...
Track C1 established the original evidence. Track C2.1 subsequently resolved the ML horizon-leakage finding only; all other confirmed findings remain open.
...
| -- | -- | -- | -- | -- |
| C1-RSI-1 | Rising RSI | DEFECT_CONFIRMED | `IndicatorTest.test_rsi_continuous_gains_reaches_100_after_warmup` | Make warm gain-only RSI finite 100. |
| C1-RSI-2 | Falling RSI | NOT_REPRODUCED | `IndicatorTest.test_rsi_continuous_losses_reaches_0_after_warmup` | None. |
| C1-RSI-3 | Flat RSI | CURRENT_BEHAVIOR_CONFIRMED | `IndicatorTest.test_rsi_flat_series_currently_remains_nan_after_warmup` | Final contract remains undecided for Track C2. |
...
The current inventory contains exactly nine tests:

1. `tests.test_indicators.IndicatorTest.test_rsi_continuous_gains_reaches_100_after_warmup`
2. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_risk_snapshot_rejects_all_nonfinite_numeric_fields`
3. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_risk_monetary_limits_reject_all_nonfinite_values`
4. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_simulated_fill_and_portfolio_reject_all_nonfinite_money`
5. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_portfolio_arithmetic_rejects_nonfinite_fill_before_contamination`
6. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_backtest_rejects_all_nonfinite_parameters_and_prices`
7. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_guard_adapter_rejects_all_nonfinite_positive_reference_prices`
8. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_ai_walk_forward_runtime_exception_returns_nonzero_exit_status`
9. `tests.test_track_c1_research_correctness.TrackC1ResearchCorrectnessTest.test_analyze_cli_runtime_validation_returns_nonzero_exit_status`

The ML leakage test is intentionally absent because it passes after Track C2.1.

## Direct and inferred impact

Direct tests show rising RSI can leave Analyze without a latest row, non-finite values cross the stated boundaries, and the original ML split leaked future label dates. Scanner, Daily Report, ML Dataset, and Parameter Sweep effects remain code-path inference unless separately integration-tested.
...
Track C1 did not change production behavior. Track C2.1 changed only ML split leakage and did not alter RSI, finite validation, metrics, CLI behavior, targets, models, reports, artifacts, strategies, data sources, or broker integration.
