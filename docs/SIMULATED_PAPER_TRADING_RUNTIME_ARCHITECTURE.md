...
```text
twstock simulated-portfolio-trading CLI Flags
  (--max-order-notional, --max-position-quantity,
   --max-position-notional, --max-total-exposure)
                   │
                   ▼
  CLI Argument Parsing & Facade Boundary Validation
  (rejects negative, NaN, inf, bool, numeric strings)
                   │
                   ▼
  Facade Creates State-Bound Composite Provider Adapter
  (PortfolioRiskGuardProvider instance with runtime_state binding)
                   │
                   ▼
  Stepper Supplies Candidate Reference-Price Context
  (passes candidate bar Open price into risk provider context)
                   │
                   ▼
  Composite Provider Evaluates All Guard Sources & Deduplicates
  (evaluates fixed decision, risk limits, and custom provider)
                   │
                   ▼
   Existing Rejection & Audit Log Pipeline
  (records SimulatedOrderRejection with stable reason ID)
                   │
                   ▼
     Schema v1 JSON Artifact Output
  (native support for rejections & audit_log without schema change)
```
...
- **Planned Phase 53.6B Components**:
  - Portfolio risk limit configuration models
  - State-bound `PortfolioRiskGuardProvider` adapter class
  - Stepper candidate reference-price context wiring
  - Latest observed Open price mark dictionary (`latest_observed_open_by_symbol`)
  - Evaluate-all composite guard orchestration
  - Multi-symbol CLI risk flags parsing and routing
...
### 4. Exposure-Increasing Order Policy
...
- `SELL` / exit orders decrease portfolio exposure and position sizes: BYPASS portfolio risk limits so risk caps never block or prevent exiting existing long positions.

### 5. Reference Price & Price Basis Policy
- Candidate reference price at signal time is the candidate bar's Open price (`open_price`) passed during chronological evaluation on Bar $N$.
...
### 6. Quantity and Notional Calculation Formulas
...
### 8. Enforcement Layer Architecture & State-Bound Composite Provider
...
### 10. Stable Rejection Identifiers & Audit Log Alignment
...
- **Planned Test Matrix**:
  - Validation: omitted, zero, positive, negative, bool, numeric string, NaN, infinity. `max_position_quantity` rejects exact float `1000.0` and fractional float.
  - Limits: order notional, position quantity, position notional, total exposure, SELL bypass.
  ...
  - Integration: composite guards, CLI routing, schema v1 export/read-back, offline artifact CLI compatibility, regression testing.
