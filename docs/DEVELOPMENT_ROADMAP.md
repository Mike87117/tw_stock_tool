...
**Status Update:** Neutral simulated order rejection capture has been added to the paper trading engine. Rejection data is now being exposed through report, export, and serialization paths. The first three initial risk limits are now available in the historical simulated CLI. `check_max_total_exposure(...)` exists as a pure risk rule. `max_total_exposure` is now available in `SimulatedPaperTradingRiskConfig`. Risk Builder now invokes `check_max_total_exposure(...)`. The previous adapter snapshot only represented the candidate symbol's current position. Phase 48.6 created the portfolio-wide exposure valuation provider. Phase 48.7 allows direct adapter injection. The adapter still preserves its previous single-symbol fallback when no provider is supplied. Guard Builder now requires and injects a portfolio-wide exposure provider when total-exposure risk is configured. Both simulated paper trading workflow helpers now optionally accept and pass through a portfolio exposure provider. Omitting the provider preserves the legacy adapter fallback. Workflow passthrough is now complete. The historical CLI still does not construct a portfolio exposure provider. `--max-total-exposure` still does not exist. Total-exposure enforcement is still not user-facing. The current historical engine still creates a fresh single-symbol simulated portfolio for each run. This phase does not create built-in multi-symbol portfolio state. A runtime architecture audit is required before exposing a portfolio-wide CLI flag. The project remains research-only, simulated-only and offline-only. No Broker or live trading capability exists.
