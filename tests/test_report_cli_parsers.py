...
class ReportCliParserTest(unittest.TestCase):
    ...
    def test_report_outputs_support_omitted_bare_and_explicit_values(self) -> None:
        for name, parse_args in PARSERS:
            for option, explicit in (("--output-md", "report.md"), ("--output-excel", "report.xlsx")):
                with self.subTest(cli=name, option=option):
                    field = option[2:].replace("-", "_")
                    self.assertIsNone(getattr(parse_args(MINIMUM_ARGS), field))
                    self.assertEqual(getattr(parse_args([*MINIMUM_ARGS, option]), field), "")
                    self.assertEqual(getattr(parse_args([*MINIMUM_ARGS, option, explicit]), field), explicit)
    ...
    def test_invalid_values_exit_two_before_runtime(self) -> None:
        cases = (
            ("backtest", backtest_report._parse_args, [*MINIMUM_ARGS, "--short-window", "bad"]),
            ("parameter sweep", parameter_sweep_report._parse_args, [*MINIMUM_ARGS, "--ma-short-windows", "5,bad"]),
            ("walk forward", walk_forward_report._parse_args, [*MINIMUM_ARGS, "--fee-rate", "bad"]),
        )
    ...
    def test_help_exposes_required_options(self) -> None:
        for name, parse_args in PARSERS:
            output = StringIO()
            ...
            for option in ("--stock", "--strategy", "--output-md", "--output-excel"):
