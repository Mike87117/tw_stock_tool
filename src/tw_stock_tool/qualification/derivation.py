"""Canonical pure derivation for strategy qualification outcomes."""

from __future__ import annotations

from tw_stock_tool.qualification.findings import finding_reason_codes, normalize_findings
...
def _finding(
    ...
    *,
    ...
    return QualificationFinding(
        code=code,
        severity=resolve_finding_severity(request.policy, code),
        scope="aggregate",
        message=message,
...
def derive_qualification_outcome(
    request: StrategyQualificationRequest,
) -> tuple[tuple[QualificationFinding, ...], PromotionDecision]:
    """Derive canonical findings and promotion decision from validated evidence."""
    ...
    if not is_supported_qualification_policy(policy):
        ...
        if metrics.symbol_concentration_pct > policy.maximum_symbol_concentration_pct:
            findings.append(
                _finding(
                    request,
    ...
    normalized = normalize_findings(findings, policy)
    ...
    decision = PromotionDecision(
        state=state,
        reason_codes=finding_reason_codes(normalized, policy),
    )
    return normalized, decision


def evaluate_derived_request(
    request: StrategyQualificationRequest,
) -> StrategyQualificationResult:
    findings, decision = derive_qualification_outcome(request)
...
__all__ = ["derive_qualification_outcome", "evaluate_derived_request"]
